...
async function apiGet(path) {
  ...
  try { const r = await fetch(API_URL + path, { headers }); return await r.json(); } catch (e) { return null; }
}
async function apiPost(path, body) {
  const headers = { 'Content-Type': 'application/json' };
  const token = getToken();
  if (token) headers['Authorization'] = 'Bearer ' + token;
  try { const r = await fetch(API_URL + path, { method: 'POST', headers, body: JSON.stringify(body) }); return await r.json(); } catch (e) { return null; }
}
async function apiPut(path, body) {
  const headers = { 'Content-Type': 'application/json' };
  const token = getToken();
  if (token) headers['Authorization'] = 'Bearer ' + token;
  try { const r = await fetch(API_URL + path, { method: 'PUT', headers, body: JSON.stringify(body) }); return await r.json(); } catch (e) { return null; }
}
async function apiDelete(path) {
  const headers = { 'Content-Type': 'application/json' };
  const token = getToken();
  if (token) headers['Authorization'] = 'Bearer ' + token;
  try { const r = await fetch(API_URL + path, { method: 'DELETE', headers }); return await r.json(); } catch (e) { return null; }
}
...
function renderNavbar() {
  ...
  container.innerHTML = '<a href="/" class="navbar-brand">Tweadup</a><nav class="navbar-nav">' + navLinks + '</nav>' + authSection + '<button class="mobile-menu-btn" onclick="toggleMobileMenu()">Menu</button>';
...
function isCoursAccessible(formationId, coursOrdre) { if (isAdmin()) return true; if (coursOrdre === 1) return true; const prev = getLocalProgressions().find(p => p.formation_id === formationId && parseInt(p.cours_id.split('_c')[1]) === coursOrdre - 1); return prev && prev.quiz_reussi; }
...
document.addEventListener('DOMContentLoaded', function() { renderNavbar(); renderFooter(); });
