// ============================================
// API BRIDGE - utilise Supabase au lieu du backend Node.js
// ============================================
...
// Mapping des chemins API vers les fonctions Supabase
async function apiGet(path) {
  if (!supabase) return null;
  try {
    if (path === '/api/formations') return await sbGetFormations();
    if (path.startsWith('/api/formations/') && path.endsWith('/cours')) {
      const id = path.split('/')[3];
      return await sbGetCours(id);
    }
    if (path.startsWith('/api/formations/')) {
      const id = path.split('/')[3];
      return await sbGetFormation(id);
    }
    ...
    if (path === '/api/notifications') return await sbGetNotifications();
    if (path === '/api/admin/stats') return await sbGetAdminStats();
    if (path === '/api/admin/certificats') return await sbGetAllCertificats();
    if (path === '/api/admin/etudiants') return await sbGetEtudiants();
    if (path === '/api/admin/formations') return await sbGetFormations();
...
async function apiPost(path, body) {
  ...
  try {
    ...
    if (path === '/api/admin/formations') return await sbCreateFormation(body);
    if (path === '/api/admin/cours') return await sbCreateCours(body);
    return null;
...
async function apiPut(path, body) {
  if (!supabase) return null;
  try {
    if (path.startsWith('/api/inscriptions/') && path.includes('/traiter')) {
      const id = path.split('/')[3];
      return await sbUpdateInscription(id, body.statut);
    }
    if (path === '/api/progressions') {
      return await sbSaveProgression(body.formation_id, body.cours_id, body.statut, body.quiz_reussi, body.score_quiz);
    }
    if (path.startsWith('/api/notifications/')) {
      const id = path.split('/')[3];
      return await sbMarkNotificationRead(id);
    }
    return null;
  } catch (e) { console.error('apiPut error:', e); return null; }
}

async function apiDelete(path) {
  if (!supabase) return null;
  try {
    if (path.startsWith('/api/admin/formations/')) {
      const id = path.split('/')[4];
      await sbDeleteFormation(id);
      return { ok: true };
    }
    return null;
  } catch (e) { console.error('apiDelete error:', e); return null; }
}
